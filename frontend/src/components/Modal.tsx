...
export function Modal({ open, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return createPortal(
    ...
    >
      <div
        className={`glass-panel w-full ${WIDTHS[size]} max-h-[90vh] overflow-y-auto p-6 animate-scale-in`}
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-2">
          ...
          <button
            onClick={onClose}
            className="shrink-0 rounded-lg p-1 text-slate-400 transition-all hover:rotate-90 hover:bg-white/10 hover:text-white"
